"""
Module Name:
Description: To perform cross-subject validation on the MODMA EEG dataset,
a 10x10 nested cross-validation is used for data partitioning.
Author: YYZ
Date: 2024-11-25
Version: 1.0
"""
...
save_model_file = current_dir + '/model_for_10_10_CV'
# 确保输出目录存在
os.makedirs(save_model_file, exist_ok=True)

### ========================= Load data and Split data for 10x10 nested cross-validation ===============================================###
data_type = 'MODMA_EEG'
...
for i_out, (train_index, test_index) in enumerate(kf_out.split(input_list)):
    ...
    for i_in, (train_index_in, valid_index_in) in enumerate(kf_in.split(train_fold)):
        ...
        print(f"Out Fold {i_out + 1} In Fold {i_in + 1}:")
        print(f"  Train subjects: {train_fold_in}")
        print(f"  Valid subjects : {valid_fold_in}")

        train_eegs, train_labels = load_designated_subjects_data_from_MODMA(input_folder, train_fold_in)
        valid_eegs, valid_labels = load_designated_subjects_data_from_MODMA(input_folder, valid_fold_in)
        ...
        for i in range(valid_eegs.shape[0]):
            eeg = valid_eegs[i]
            psd, de, se = extract_frequency_bands(eeg, fs=fs, nperseg=nperseg, noverlap=noverlap)
            feature = np.stack((psd, de, se), axis=0)
            valid_datas.append(feature)  # (8012, 3, 128, 5) [batch_size, 3, eeg_channels, features]
        valid_datas = np.array(valid_datas)
        ...
        # model
        model = Adopt_Gnn_PSD_DE_SE(dim, eeg_channels, subgraph_size, device, alpha_PSD, alpha_DE, alpha_SE, beta_PSD,
                                    beta_DE, beta_SE, gcn_depth, layers, num_classes=num_classes).to(device)
        # 优化器
        ...
        # 设置每一折的最佳模型保存位置
        folder_bestmodel_file = os.path.join(save_model_file + f"/Out{i_out}_In{i_in}fold_model.pth")
        ...
        for iter in range(train_epochs):
            ...
            print(
                f"Train: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- acc: {all_train_accuracy / len(train_data):.4f} ---- loss: {all_train_loss / len(train_data):.4f}\n")
            ...
            print(
                f"Eval: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- acc: {all_eval_accuracy / len(eval_data):.4f} ---- loss: {all_eval_loss / len(eval_data):.4f}\n")
            ...
            print(f"Eval: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- accuracy: {eval_accuracy:.4f}\n")
            print(f"Eval: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- recall(sensitivity): {eval_recall:.4f}\n")
            print(f"Eval: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- specificity: {eval_specificity:.4f}\n")
            print(f"Eval: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- precision: {eval_precision:.4f}\n")
            print(f"Eval: Epoch: {iter} ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- f1: {eval_f1:.4f}\n")
    ...
    print(f"第 {i_out} 外折第 {i_in} 内折交叉训练的模型验证集上的平均表现如下：\n")
    ...
    # 准备test数据
    test_eegs, test_labels = load_designated_subjects_data_from_MODMA(input_folder, test_fold)
    test_datas = []
    ...
    for i_in in range(folds_in):
        model = Adopt_Gnn_PSD_DE_SE(dim, eeg_channels, subgraph_size, device, alpha_PSD, alpha_DE, alpha_SE, beta_PSD,
                                    beta_DE, beta_SE, gcn_depth, layers, num_classes=num_classes).to(device)
        model_file = os.path.join(save_model_file + f"/Out{i_out}_In{i_in}fold_model.pth")  # 加载训练好的模型
        ...
        print(f"Test: ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- accuracy: {test_accuracy:.4f}\n")
        print(f"Test: ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- recall(sensitivity): {test_recall:.4f}\n")
        print(f"Test: ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- specificity: {test_specificity:.4f}\n")
        print(f"Test: ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- precision: {test_precision:.4f}\n")
        print(f"Test: ---- 外折kfold:{i_out + 1}----内折kfold: {i_in + 1} ---- f1: {test_f1:.4f}\n")

        ConfusionMatrixDisplay.from_predictions(test_targets,
                                                test_preds,
                                                display_labels=["MDD", "HD"],
                                                # 第一行和第一列对应于标签 0（即 "MDD"） 第二行和第二列对应于标签 1（即 "HD"）
                                                cmap=plt.cm.Reds,
                                                colorbar=True)
        # 设置标题
        model_safe_name = re.sub(r'[<>:"/\\|?*]', '', model.__class__.__name__)
        plt.title(f'Confusion Matrix for {model_safe_name} of 外折kfold:{i_out + 1} 内折kfold: {i_in + 1}')
        plt.show()
    ...
    print(f"第 {i_out} 外折第 {i_in} 内折交叉训练的模型测试集上的平均表现如下：")
    print(f"Accuracy: Average = {test_kfold_acc_avg * 100:.2f}%, Standard Deviation = {test_kfold_acc_std * 100:.2f}%")
    print(
        f"Recall: Average = {test_kfold_recall_avg * 100:.2f}%, Standard Deviation = {test_kfold_recall_std * 100:.2f}%")
    print(
        f"Specificity: Average = {test_kfold_specificity_avg * 100:.2f}%, Standard Deviation = {test_kfold_specificity_std * 100:.2f}%")
    print(
        f"Precision: Average = {test_kfold_precision_avg * 100:.2f}%, Standard Deviation = {test_kfold_precision_std * 100:.2f}%")
    print(f"F1 Score: Average = {test_kfold_f1_avg * 100:.2f}%, Standard Deviation = {test_kfold_f1_std * 100:.2f}%\n")

    ### ========================= 保存测试集结果 ===============================================###
    # 准备每一折的结果数据
    fold_results = []
    for i in range(folds_in):
        fold_results.append({
            "Fold": i + 1,
    ...
    save_report_path = current_dir + '/result_for_10_10_CV/'
    # 确保输出目录存在
    os.makedirs(save_report_path, exist_ok=True)

    report_file = f"{save_report_path}test_report_{data_type}_{current_date}.md"
